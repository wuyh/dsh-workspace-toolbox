...
type Rule = [pattern: RegExp, color: string]

const HL_RULES: Record<string, Rule[]> = {
  js: [
    [/\/\/.*|\/\*.*\*\//, '#6a9955'],
    [/'(?:\\.|[^'\\\n])*'|"(?:\\.|[^"\\\n])*"|`(?:\\.|[^`\\])*`/, '#ce9178'],
...
const HL_FLAGS: Record<string, string> = { sql: 'gi', ini: 'gi' }

interface CompiledHl {
  re: RegExp
  colors: string[]
}

const HL_CACHE: Record<string, CompiledHl | null> = {}

/** 按语言编译（缓存）合并正则：一个捕获组对应一条规则。 */
export function hlFor(lang: string): CompiledHl | null {
  if (HL_CACHE[lang] !== undefined) return HL_CACHE[lang] ?? null
  ...
  const compiled: CompiledHl = { re, colors: rules.map((r) => r[1]) }
...
export function tokenizeLine(line: string, hl: CompiledHl): Token[] {
  const tokens: Token[] = []
  let last = 0
  hl.re.lastIndex = 0
