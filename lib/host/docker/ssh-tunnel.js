...
export function connectSsh(target, auth) {
    return new Promise((resolve, reject) => {
        const client = new Client();
        const onReady = () => { cleanup(); resolve(client); };
 ...
 */
export function dockerSocket(conn, socketPath = '/var/run/docker.sock') {
    const socket = new Duplex({
        read() { },
        write(chunk, _encoding, cb) {
            if (channel !== null)
                channel.write(chunk, cb);
            else
    ...
    conn.openssh_forwardOutStreamLocal(socketPath, (err, ch) => {
        ...
        channel = ch;
        ch.on('data', (d) => { socket.push(d); });
        ch.on('end', () => { socket.push(null); });
        ch.on('close', () => { socket.push(null); });
        ch.on('error', (e) => { socket.destroy(e); });
        for (const w of pending)
            ch.write(w.chunk, w.cb);
        pending.length = 0;
        socket.emit('connect');
    });
    const socketLike = socket;
