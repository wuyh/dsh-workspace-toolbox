/**
 * Vite 服务的 HTTP 路由：工作区项目扫描、dev server 启动/停止、状态轮询。
 *
 * 所有路由挂在 /dsh-workspace-toolbox/vite/* 下，方法 GET/POST。
 */
...
export function registerViteRoutes(ctx: Context): void {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (key === '') return json(res, 200, { ok: false, error: 'INVALID_KEY' })
