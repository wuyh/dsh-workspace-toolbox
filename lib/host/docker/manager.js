...
export async function connect(spec) {
    if (spec.kind === 'local') {
        ...
        if (host === '' || username === '')
            throw new Error('请填写 SSH 主机与用户名');
        if (spec.authKind === 'password' && spec.password === undefined) {
            throw new Error('请填写 SSH 密码');
        }
        if (spec.authKind === 'key' && (spec.keyPath === undefined || spec.keyPath.trim() === '')) {
            throw new Error('请填写私钥文件路径');
        }
        const conn = await connectSsh({ host, port, username }, spec.authKind === 'password'
            ? { kind: 'password', password: spec.password }
            : { kind: 'key', keyPath: spec.keyPath, passphrase: spec.passphrase });
        const backend = { kind: 'ssh', conn };
        try {
            const info = await docker.version(backend);
...
/** 断开并（可选）删除元数据。 */
export function disconnect(id, forget = false) {
    const entry = runtime.get(id);
    if (entry !== undefined && entry.sshClient !== undefined)
        entry.sshClient.end();
    runtime.delete(id);
    if (forget) {
...
/** 取得连接对应的后端；未连接时抛错。 */
export function backendOf(id) {
    const entry = runtime.get(id);
    if (entry === undefined)
        throw new Error('连接未建立，请先连接');
    return entry.backend;
}
function withoutSecrets(spec) {
    const copy = { id: spec.id, kind: spec.kind, name: spec.name };
    if (spec.kind === 'ssh') {
        copy.host = spec.host;
        copy.port = spec.port;
