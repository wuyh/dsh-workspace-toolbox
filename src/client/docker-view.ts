...
function fmtSize(n: number | undefined): string {
  ...
  if (n < 1048576 * 1024) return (n / 1048576).toFixed(1) + ' MB'
  return (n / 1048576 / 1024).toFixed(2) + ' GB'
}

function repoTagOf(image: DockerImageRow): string {
  const tags = image.RepoTags ?? []
  if (tags.length > 0) return tags[0] ?? ''
  return (image.Id ?? '').replace('sha256:', '').slice(0, 12)
}

function nameOf(container: DockerContainerRow): string {
  return (container.Names[0] ?? container.Id).replace(/^\//, '')
}

function portsOf(container: DockerContainerRow): string {
  const parts = container.Ports.map((p) => (p.PublicPort !== undefined ? `${p.PublicPort}->${p.PrivatePort}` : String(p.PrivatePort)))
  return parts.join(', ')
}

const sessionStores = new Map<string, { tab: string, conn: string }>()

function sessionStore(sid: string): { tab: string, conn: string } {
...
export function DockerPanel(props: DockerViewProps): ReactNode {
  const sid = typeof props.sessionId === 'string' ? props.sessionId : ''
  const store = sessionStore(sid)

  const [connections, setConnections] = React.useState<DockerConnectionView[] | null>(null)
  const [conn, setConn] = React.useState<string>(store.conn)
  const [tab, setTab] = React.useState<string>(store.tab)
  const [images, setImages] = React.useState<{ phase: 'idle' | 'loading' | 'ready' | 'error', rows: DockerImageRow[], error: string }>({ phase: 'idle', rows: [], error: '' })
  const [containers, setContainers] = React.useState<{ phase: 'idle' | 'loading' | 'ready' | 'error', rows: DockerContainerRow[], error: string }>({ phase: 'idle', rows: [], error: '' })
  const [jobs, setJobs] = React.useState<DockerJob[]>([])
  const [projects, setProjects] = React.useState<DockerProject[] | null>(null)
  ...
  const [logs, setLogs] = React.useState<{ containerId: string, text: string, phase: 'loading' | 'ready' | 'error' } | null>(null)

  React.useEffect(() => { store.conn = conn }, [conn])
  React.useEffect(() => { store.tab = tab }, [tab])

  const refreshConnections = (): void => {
    dockerRpc.connections().then((res) => { if (res.ok) setConnections(res.connections) }).catch(() => {})
  }
  const refreshImages = (): void => {
    setImages((s) => ({ ...s, phase: 'loading' }))
    dockerRpc.images(conn).then((res) => {
      if (res.ok) setImages({ phase: 'ready', rows: res.images, error: '' })
      else setImages({ phase: 'error', rows: [], error: res.error })
  ...
  const refreshContainers = (): void => {
    ...
    dockerRpc.containers(conn).then((res) => {
      if (res.ok) setContainers({ phase: 'ready', rows: res.containers, error: '' })
      else setContainers({ phase: 'error', rows: [], error: res.error })
    }).catch(() => setContainers({ phase: 'error', rows: [], error: 'UNKNOWN' }))
  }
  const refreshJobs = (): void => {
    dockerRpc.jobs().then((res) => { if (res.ok) setJobs(res.jobs) }).catch(() => {})
  }

  React.useEffect(() => {
    refreshConnections()
    dockerRpc.projects(sid).then((res) => { if (res.ok) setProjects(res.projects) }).catch(() => {})
    refreshJobs()
  }, [sid])
  ...
  const doDisconnect = (id: string, forget: boolean): void => {
    dockerRpc.disconnect(id, forget).then(() => {
      refreshConnections()
      if (id === conn) setConn('local')
    }).catch(() => {})
  }
  ...
  const doBuild = (): void => {
    if (buildDir === '' || buildTag.trim() === '') return
    dockerRpc.build(sid, conn, buildDir, buildTag.trim()).then((res) => { if (res.ok) { setTab('jobs'); refreshJobs() } }).catch(() => {})
  }
  const doPull = (): void => {
    if (pullImage.trim() === '') return
    dockerRpc.pull(conn, pullImage.trim()).then((res) => { if (res.ok) { setTab('jobs'); refreshJobs() } }).catch(() => {})
  }
  const doRun = (): void => {
    if (runImage.trim() === '') return
    const ports = runPorts.split(/[,，\s]+/).filter((p) => p !== '')
    const env = runEnv.split(/[,，\n]+/).filter((e) => e.trim() !== '')
    dockerRpc.run(conn, runImage.trim(), runName.trim(), ports, env).then((res) => { if (res.ok) { setTab('jobs'); refreshJobs() } }).catch(() => {})
  }
  const doStop = (id: string): void => { dockerRpc.stop(conn, id).then(() => refreshJobs()).catch(() => {}) }
  const doRemove = (id: string): void => { dockerRpc.remove(conn, id).then(() => { refreshJobs(); refreshContainers() }).catch(() => {}) }
  const doRemoveImage = (id: string): void => { dockerRpc.removeImage(conn, id).then(() => { refreshJobs(); refreshImages() }).catch(() => {}) }
  const doLogs = (id: string): void => {
    setLogs({ containerId: id, text: '', phase: 'loading' })
    dockerRpc.logs(conn, id).then((res) => {
      if (res.ok) setLogs({ containerId: id, text: res.logs, phase: 'ready' })
      else setLogs({ containerId: id, text: res.error, phase: 'error' })
    }).catch(() => setLogs({ containerId: id, text: '读取日志失败', phase: 'error' }))
  }

  const setField = (key: string, value: string): void => setForm((f) => ({ ...f, [key]: value }))
  ...
  const connectionRows = (connections ?? []).map((c) => {
    ...
    return React.createElement('div', { key: c.id, className: c.id === conn ? 'dshdc-row dshdc-row-active' : 'dshdc-row', onClick: () => { setRowError(''); setConn(c.id) } },
      ...
      React.createElement('div', { className: 'dshdc-row-main' },
        ...
        React.createElement('div', { className: 'dshdc-row-sub' }, c.kind === 'local' ? '本机 unix socket' : `${c.username}@${c.host}:${c.port ?? 22}` + engine),
      ),
  ...
  if (currentConn !== null && !currentConn.connected) {
    ...
    else content = images.rows.map((img) => React.createElement('div', { key: img.Id, className: 'dshdc-list-row' },
      React.createElement('span', { className: 'dshdc-list-name' }, repoTagOf(img)),
      React.createElement('span', { className: 'dshdc-list-meta' }, fmtSize(img.Size)),
      React.createElement('span', { className: 'dshdc-list-id' }, (img.Id ?? '').replace('sha256:', '').slice(0, 12)),
      React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => doRemoveImage(img.Id) }, '删除'),
    ))
  } else if (tab === 'containers') {
    if (containers.phase === 'loading') content = React.createElement('div', { className: 'dshdc-center' }, '加载中…')
    else if (containers.phase === 'error') content = React.createElement('div', { className: 'dshdc-center' }, '加载失败：' + containers.error)
    else if (containers.rows.length === 0) content = React.createElement('div', { className: 'dshdc-center' }, '暂无容器')
    else content = containers.rows.map((c) => React.createElement('div', { key: c.Id },
      React.createElement('div', { className: 'dshdc-list-row' },
        React.createElement('span', { className: 'dshdc-list-name' }, nameOf(c)),
        React.createElement('span', { className: 'dshdc-list-meta' }, c.Image),
      ...
      logs !== null && logs.containerId === c.Id
    ...
    if (jobs.length === 0) content = React.createElement('div', { className: 'dshdc-center' }, '暂无任务')
      ...
      React.createElement('div', { className: 'dshdc-job-head' },
        React.createElement('span', { className: 'dshdc-job-label' }, job.label),
        React.createElement('span', { className: 'dshdc-job-status' }, job.status === 'running' ? '⏳ 进行中' : job.status === 'ok' ? '✅ 完成' : '❌ 失败'),
        job.detail !== undefined ? React.createElement('span', { className: 'dshdc-list-id' }, job.detail.slice(0, 12)) : null,
      ),
      React.createElement('pre', { className: 'dshdc-log' }, job.log === '' ? '（无输出）' : job.log),
    ))
  }
  ...
  return React.createElement('div', { className: 'dshdc-main' },
      React.createElement('div', { className: 'dshdc-side' },
        React.createElement('div', { className: 'dshdc-side-head' },
          ...
          React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => setAddOpen((v) => !v) }, addOpen ? '收起' : '＋ 远程'),
        ),
        addOpen ? React.createElement('div', { className: 'dshdc-form' },
          input(form.name, '名称（可选）', (v) => setField('name', v), '100%'),
          input(form.host, '服务器地址', (v) => setField('host', v), '100%'),
          input(form.port, 'SSH 端口', (v) => setField('port', v), '100%'),
          input(form.username, '用户名', (v) => setField('username', v), '100%'),
          React.createElement('select', {
            className: 'dshdc-input', style: { width: '100%' }, value: form.authKind,
            onChange: (e: { target: { value: string } }) => setField('authKind', e.target.value),
          },
            React.createElement('option', { value: 'password' }, '密码认证'),
            React.createElement('option', { value: 'key' }, '私钥认证'),
          ),
          ...
          form.authKind === 'key'
            ...
            : null,
          connectError !== '' ? React.createElement('div', { className: 'dshdc-error' }, connectError) : null,
      ...
      React.createElement('div', { className: 'dshdc-body' },
        ...
        React.createElement('div', { className: 'dshdc-actions' },
          ...
          React.createElement('select', {
            ...
            onChange: (e: { target: { value: string } }) => setBuildDir(e.target.value),
          },
            React.createElement('option', { value: '' }, '选择项目…'),
            (projects ?? []).map((p) => React.createElement('option', { key: p.dir, value: p.dir }, p.name)),
          ),
          input(buildTag, '镜像 tag', setBuildTag, 160),
          React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: buildDir === '' || buildTag.trim() === '', onClick: doBuild }, '构建'),
          React.createElement('span', { className: 'dshdc-sep' }),
          React.createElement('span', { className: 'dshdc-actions-label' }, '拉取'),
          input(pullImage, '镜像名:tag', setPullImage, 160),
          React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: pullImage.trim() === '', onClick: doPull }, '拉取'),
          React.createElement('span', { className: 'dshdc-sep' }),
          React.createElement('span', { className: 'dshdc-actions-label' }, '运行'),
          input(runImage, '镜像名:tag', setRunImage, 140),
          input(runName, '容器名', setRunName, 100),
          input(runPorts, '端口 8080:80,3000:3000', setRunPorts, 150),
