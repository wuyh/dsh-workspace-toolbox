...
import { addWorkspaceProject, listWorkspaceProjects, removeWorkspaceProject, scanAllDockerfileModules } from './projects.js';
...
export function registerDockerRoutes(ctx) {
    ...
    // POST /docker/projects/add { session, path } → 添加项目（相对/绝对路径）并持久化
    ctx.effect(() => ctx.webServer.register({
        kind: 'exact',
        ...
        handler: async (req, res) => {
            try {
                ...
                if ('error' in resolved)
                    return json(res, 200, { ok: false, error: resolved.error });
                const result = addWorkspaceProject(resolved.rootPath, str(body.path));
                if (!result.ok)
    ...
    // POST /docker/projects/remove { session, path } → 从已添加列表移除
    ctx.effect(() => ctx.webServer.register({
        kind: 'exact',
        path: PREFIX + '/projects/remove',
        handler: async (req, res) => {
            try {
                const body = await readJsonBody(req);
                const resolved = await resolveRoot(ctx, str(body.session));
                if ('error' in resolved)
                    return json(res, 200, { ok: false, error: resolved.error });
                const result = removeWorkspaceProject(resolved.rootPath, str(body.path));
                if (!result.ok)
                    return json(res, 200, { ok: false, error: result.error });
                json(res, 200, { ok: true, projects: result.projects });
            }
            catch (error) {
                json(res, 200, { ok: false, error: 'REMOVE_FAILED', message: messageOf(error) });
            }
        },
    }));
        ...
        handler: async (req, res) => {
            try {
                ...
                if (spec === undefined || typeof spec !== 'object')
                    return json(res, 200, { ok: false, error: 'INVALID_SPEC' });
    ...
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (image === '')
                return json(res, 200, { ok: false, error: 'INVALID_IMAGE' });
    ...
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                if (dir === '' || tag === '')
                    return json(res, 200, { ok: false, error: 'INVALID_ARGS' });
    ...
    ctx.effect(() => ctx.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (id === '')
                return json(res, 200, { ok: false, error: 'INVALID_ID' });
