...
function parseControl(data) {
    ...
    for (const kind of ['init', 'resize']) {
        if (body.startsWith(kind)) {
            try {
                const parsed = JSON.parse(body.slice(kind.length));
 ...
 */
function pipeStream(ws, stream, setSize) {
    let settled = false;
    const settle = () => {
        if (settled)
            return;
...
export function registerTerminalUpgrade(ctx) {
 ...
 */
async function handleContainerTerminal(ws, backend, container) {
    let execId;
    try {
