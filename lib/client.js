window.__ModuleLoader__.load({
  ...
  function DockerPanel(props) {
      ...
      const refreshConnections = () => {
          docker_rpc_js_1.dockerRpc.connections().then((res) => { if (res.ok)
              setConnections(res.connections); }).catch(() => { });
      };
      const refreshImages = () => {
          setImages((s) => ({ ...s, phase: 'loading' }));
          docker_rpc_js_1.dockerRpc.images(conn).then((res) => {
              if (res.ok)
      ...
      const refreshContainers = () => {
          ...
          docker_rpc_js_1.dockerRpc.containers(conn).then((res) => {
              if (res.ok)
      ...
      const refreshJobs = () => {
          docker_rpc_js_1.dockerRpc.jobs().then((res) => { if (res.ok)
              setJobs(res.jobs); }).catch(() => { });
      };
      React.useEffect(() => {
          refreshConnections();
          docker_rpc_js_1.dockerRpc.projects(sid).then((res) => { if (res.ok)
      ...
      const doConnect = () => {
          setConnecting(true);
          setConnectError('');
          docker_rpc_js_1.dockerRpc.connect({
      ...
      const doDisconnect = (id, forget) => {
          docker_rpc_js_1.dockerRpc.disconnect(id, forget).then(() => {
              refreshConnections();
              if (id === conn)
                  setConn('local');
          }).catch(() => { });
      ...
      const doBuild = () => {
          if (buildDir === '' || buildTag.trim() === '')
              return;
          docker_rpc_js_1.dockerRpc.build(sid, conn, buildDir, buildTag.trim()).then((res) => { if (res.ok) {
              setTab('jobs');
              refreshJobs();
          } }).catch(() => { });
      };
      const doPull = () => {
          if (pullImage.trim() === '')
              return;
          docker_rpc_js_1.dockerRpc.pull(conn, pullImage.trim()).then((res) => { if (res.ok) {
              setTab('jobs');
              refreshJobs();
          } }).catch(() => { });
      };
      const doRun = () => {
          ...
          docker_rpc_js_1.dockerRpc.run(conn, runImage.trim(), runName.trim(), ports, env).then((res) => { if (res.ok) {
              setTab('jobs');
              refreshJobs();
          } }).catch(() => { });
      };
      const doStop = (id) => { docker_rpc_js_1.dockerRpc.stop(conn, id).then(() => refreshJobs()).catch(() => { }); };
      const doRemove = (id) => { docker_rpc_js_1.dockerRpc.remove(conn, id).then(() => { refreshJobs(); refreshContainers(); }).catch(() => { }); };
      const doRemoveImage = (id) => { docker_rpc_js_1.dockerRpc.removeImage(conn, id).then(() => { refreshJobs(); refreshImages(); }).catch(() => { }); };
      const doLogs = (id) => {
          setLogs({ containerId: id, text: '', phase: 'loading' });
          docker_rpc_js_1.dockerRpc.logs(conn, id).then((res) => {
              if (res.ok)
      ...
      const projectRows = (projects ?? []).map((p) => React.createElement('div', {
          ...
          title: p.dockerfile,
      }, React.createElement('div', { className: 'dshdc-row-main' }, React.createElement('div', { className: 'dshdc-row-name' }, p.name), React.createElement('div', { className: 'dshdc-row-sub' }, p.dir))));
      ...
      if (tab === 'images') {
          ...
          if (containers.phase === 'loading')
              ...
              content = containers.rows.map((c) => React.createElement('div', { key: c.Id }, React.createElement('div', { className: 'dshdc-list-row' }, React.createElement('span', { className: 'dshdc-list-name' }, nameOf(c)), React.createElement('span', { className: 'dshdc-list-meta' }, c.Image), React.createElement('span', { className: 'dshdc-list-state', 'data-state': c.State }, c.State === 'running' ? '运行中' : c.Status), React.createElement('span', { className: 'dshdc-list-meta' }, portsOf(c)), React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => doLogs(c.Id) }, '日志'), c.State === 'running' ? React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => doStop(c.Id) }, '停止') : null, React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => doRemove(c.Id) }, '删除')), logs !== null && logs.containerId === c.Id
      ...
      return React.createElement('div', { className: 'dshdc-main' }, React.createElement('div', { className: 'dshdc-side' }, React.createElement('div', { className: 'dshdc-side-head' }, React.createElement('span', null, '连接'), React.createElement('button', { type: 'button', className: 'dshdc-mini', onClick: () => setAddOpen((v) => !v) }, addOpen ? '收起' : '＋ 远程')), addOpen ? React.createElement('div', { className: 'dshdc-form' }, input(form.name, '名称（可选）', (v) => setField('name', v), '100%'), input(form.host, '服务器地址', (v) => setField('host', v), '100%'), input(form.port, 'SSH 端口', (v) => setField('port', v), '100%'), input(form.username, '用户名', (v) => setField('username', v), '100%'), React.createElement('select', {
          ...
          : null, connectError !== '' ? React.createElement('div', { className: 'dshdc-error' }, connectError) : null, React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: connecting, onClick: doConnect }, connecting ? '连接中…' : '连接')) : null, React.createElement('div', { className: 'dshdc-side-list' }, connectionRows), React.createElement('div', { className: 'dshdc-side-head' }, React.createElement('span', null, '工作区项目（含 Dockerfile）')), React.createElement('div', { className: 'dshdc-side-list' }, projects === null ? React.createElement('div', { className: 'dshdc-center' }, '扫描中…') : projectRows.length > 0 ? projectRows : React.createElement('div', { className: 'dshdc-center' }, '未发现 Dockerfile 项目'))), React.createElement('div', { className: 'dshdc-body' }, React.createElement('div', { className: 'dshdc-bar' }, React.createElement('span', { className: 'dshdc-title' }, currentConn !== null ? currentConn.name : 'Docker'), currentConn !== null && currentConn.connected && currentConn.engine !== undefined && currentConn.engine.version !== undefined
          ? React.createElement('span', { className: 'dshdc-list-meta' }, 'Docker ' + currentConn.engine.version)
          : React.createElement('span', { className: 'dshdc-list-meta' }, currentConn !== null && !currentConn.connected ? '未连接' : ''), React.createElement('span', { className: 'dshdc-spacer' }), ['images', 'containers', 'jobs'].map((t) => React.createElement('button', {
          key: t, type: 'button',
          className: tab === t ? 'dshdc-btn dshdc-btn-on' : 'dshdc-btn',
          onClick: () => setTab(t),
      }, t === 'images' ? '镜像' : t === 'containers' ? '容器' : '任务'))), React.createElement('div', { className: 'dshdc-actions' }, React.createElement('span', { className: 'dshdc-actions-label' }, '构建'), React.createElement('select', {
          className: 'dshdc-input', style: { width: 170 }, value: buildDir,
          onChange: (e) => setBuildDir(e.target.value),
      }, React.createElement('option', { value: '' }, '选择项目…'), (projects ?? []).map((p) => React.createElement('option', { key: p.dir, value: p.dir }, p.name))), input(buildTag, '镜像 tag', setBuildTag, 160), React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: buildDir === '' || buildTag.trim() === '', onClick: doBuild }, '构建'), React.createElement('span', { className: 'dshdc-sep' }), React.createElement('span', { className: 'dshdc-actions-label' }, '拉取'), input(pullImage, '镜像名:tag', setPullImage, 160), React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: pullImage.trim() === '', onClick: doPull }, '拉取'), React.createElement('span', { className: 'dshdc-sep' }), React.createElement('span', { className: 'dshdc-actions-label' }, '运行'), input(runImage, '镜像名:tag', setRunImage, 140), input(runName, '容器名', setRunName, 100), input(runPorts, '端口 8080:80,3000:3000', setRunPorts, 150), input(runEnv, '环境变量 KEY=VALUE', setRunEnv, 150), React.createElement('button', { type: 'button', className: 'dshdc-btn', disabled: runImage.trim() === '', onClick: doRun }, '运行')), React.createElement('div', { className: 'dshdc-content' }, content)));
  }
  ...
  */
   ...
   */
  const core_1 = __importDefault(require(3));
  const bash_1 = __importDefault(require(4));
  const c_1 = __importDefault(require(5));
  const cpp_1 = __importDefault(require(6));
  const csharp_1 = __importDefault(require(7));
  const css_1 = __importDefault(require(8));
  const go_1 = __importDefault(require(9));
  const ini_1 = __importDefault(require(10));
  const java_1 = __importDefault(require(11));
  const javascript_1 = __importDefault(require(12));
  const json_1 = __importDefault(require(13));
  const kotlin_1 = __importDefault(require(14));
  const less_1 = __importDefault(require(15));
  const markdown_1 = __importDefault(require(16));
  const plaintext_1 = __importDefault(require(17));
  const python_1 = __importDefault(require(18));
  const rust_1 = __importDefault(require(19));
  const scss_1 = __importDefault(require(20));
  const sql_1 = __importDefault(require(21));
  const swift_1 = __importDefault(require(22));
  const typescript_1 = __importDefault(require(23));
  const xml_1 = __importDefault(require(24));
  const yaml_1 = __importDefault(require(25));
   ...
   */
  const dompurify_1 = __importDefault(require(28));
  const marked_1 = require(29);
   ...
   */
  const contract_js_1 = require(31);
   ...
   */
  const React = __importStar(require("react"));
  const docker_view_js_1 = require(2);
  const highlight_js_1 = require(26);
  const icons_js_1 = require(27);
  const markdown_js_1 = require(30);
  const rpc_js_1 = require(32);
  /** 同时打开的标签上限（超出淘汰最旧的标签）。 */
  const MAX_TABS = 10;
  const ERROR_TEXT = {
  ...
  function FilesView(props) {
      ...
      React.useEffect(() => {
          ...
          let alive = true;
          (0, rpc_js_1.readFile)(sid, activePath).then((res) => {
              if (!alive)
                  return;
              if (res.ok) {
                  setDocs((d) => ({
      ...
      function renderPreviewBody() {
          ...
          if (activeDoc.kind === 'text') {
              ...
              if ((0, highlight_js_1.isMarkdown)(activeTab.name) && mdView === 'render') {
                  const html = (0, markdown_js_1.renderMarkdown)(activeDoc.text);
                  return React.createElement('div', { className: 'dshfm-md', dangerouslySetInnerHTML: { __html: html } });
              }
              return renderCodeView(activeDoc.text, (0, highlight_js_1.detectLang)(activeTab.name));
          }
      ...
      return React.createElement('div', { className: 'dshfm-root', 'data-conversation-composer-overlay': '' }, React.createElement('div', { className: 'dshfm-bar' }, React.createElement('button', { type: 'button', className: viewMode === 'files' ? 'dshfm-btn dshfm-btn-on' : 'dshfm-btn', onClick: () => setViewMode('files') }, '文件'), React.createElement('button', { type: 'button', className: viewMode === 'docker' ? 'dshfm-btn dshfm-btn-on' : 'dshfm-btn', onClick: () => setViewMode('docker') }, 'Docker'), viewMode === 'files' ? React.createElement(React.Fragment, null, React.createElement('button', { type: 'button', className: treeVisible ? 'dshfm-btn dshfm-btn-on' : 'dshfm-btn', title: '显示/隐藏目录', onClick: () => setTreeVisible((v) => !v) }, '☰'), React.createElement('span', { className: 'dshfm-path', title: state.root }, state.root || '—'), React.createElement('input', { className: 'dshfm-input', value: filter, placeholder: '筛选…', onChange: (e) => setFilter(e.target.value) }), React.createElement('button', { type: 'button', className: 'dshfm-btn', onClick: () => setReloadKey((k) => k + 1) }, '刷新'), React.createElement('button', { type: 'button', className: 'dshfm-btn', onClick: openRoot, disabled: state.root === '' }, '打开目录')) : null), React.createElement('div', { className: 'dshfm-main' }, viewMode === 'docker' ? React.createElement('div', { className: 'dshfm-docker-wrap' }, React.createElement(docker_view_js_1.DockerPanel, { sessionId: sid })) : null, viewMode === 'files' && treeVisible ? React.createElement('div', { className: 'dshfm-tree' }, React.createElement('div', { className: 'dshfm-tree-head' }, React.createElement('button', { type: 'button', className: 'dshfm-btn dshfm-back', title: '返回上一级', disabled: currentPath === '', onClick: goUp }, '\u2190'), React.createElement('div', { className: 'dshfm-crumbs' }, renderCrumbs())), React.createElement('div', { className: 'dshfm-tree-body' }, treeBody)) : null, viewMode === 'files' ? React.createElement('div', { className: 'dshfm-preview' }, tabs.length > 0 ? React.createElement('div', { className: 'dshfm-tabs' }, renderTabs()) : null, React.createElement('div', { className: 'dshfm-preview-head' }, React.createElement('span', { className: 'dshfm-preview-name' }, activeTab === null ? '预览' : activeTab.name), activeTab === null ? null : React.createElement('span', { className: 'dshfm-preview-meta', title: activeTab.path }, activeTab.path + (activeDoc !== null && activeDoc.size > 0 ? ' · ' + fmtSize(activeDoc.size) : '')), React.createElement('span', { className: 'dshfm-preview-spacer' }), activeTab !== null && activeDoc !== null && activeDoc.kind === 'text' && (0, highlight_js_1.isMarkdown)(activeTab.name)
  ...
  exports.apply = apply;
  const React = __importStar(require("react"));
  const files_view_js_1 = require(33);
  const styles_js_1 = require(34);
