...
interface ActiveRun extends ViteRun {
  proc: ChildProcess
}
...
function toView(run: ActiveRun): ViteRun {
  const { proc: _proc, ...view } = run
  return { ...view, pid: run.proc.pid ?? null }
}

function commandLine(): { shell: string, args: string[] } {
...
function appendLog(run: ViteRun, text: string): void {
  run.log = (run.log + text).slice(-MAX_LOG)
}

const URL_RE = /https?:\/\/(localhost|127\.0\.0\.1|\[::1\]):(\d+)/

function feed(run: ActiveRun, chunk: Buffer): void {
...
export function listRuns(): ViteRun[] {
 ...
 * 已在启动/运行中时直接返回现有 run。
 */
export function startRun(key: string, name: string, dir: string, command?: string): { ok: boolean, error?: string, run?: ViteRun } {
  ...
  if (cmdLine === '') return { ok: false, error: 'EMPTY_COMMAND' }
  ...
  runs.set(key, run)
  child.stdout?.on('data', (d: Buffer) => feed(run, d))
  child.stderr?.on('data', (d: Buffer) => feed(run, d))
  ...
  child.on('exit', (code, signal) => {
    ...
    if (run.status === 'starting' || run.status === 'running') run.status = 'stopped'
...
export function stopRun(key: string): { ok: boolean, error?: string } {
  const run = runs.get(key)
  if (run === undefined) return { ok: false, error: 'NOT_RUNNING' }
  if (run.status !== 'starting' && run.status !== 'running') {
...
export function disposeAllRuns(): void {
  for (const run of [...runs.values()]) {
    if (run.status === 'starting' || run.status === 'running') stopRun(run.key)
  }
  runs.clear()
}
