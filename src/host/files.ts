/**
 * Host 侧的工作区读取原语：单层列表、全工作区搜索、带边界的文件预览。
 *
 * 安全模型：
 * - 浏览器端只收到相对路径 + 最小元数据（名称/类型/大小），绝对路径
 *   仅作为“打开文件”用（processPath），且调用方是用户本人；
 * - 所有读取都经 resolve → contains 双重工作区边界校验，`..` 逃逸与
 *   越界符号链接会被拒绝；
 * - 文本按流读取到上限即截断，图片读取有字节上限，大文件不会拖垮进程。
 */
...
export function absPath(ctx: Context, target: FsTarget): string | undefined {
  try {
...
export async function listLevel(ctx: Context, target: FsTarget, rel: string): Promise<Entry[]> {
  let entries
  try {
...
export async function searchWorkspace(ctx: Context, root: FsTarget, query: string): Promise<SearchResult> {
  const matches: Entry[] = []
  let visited = 0
  let truncated = false

  const walk = async (target: FsTarget, rel: string, depth: number): Promise<void> => {
 ...
 * 其余按二进制元数据返回。
 */
export async function readPreview(ctx: Context, target: FsTarget, rel: string, size: number): Promise<ReadResult> {
