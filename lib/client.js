window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    (function (module, exports, require) {
      ...
      // --- RPC helpers (same-origin HTTP routes served by the Host half) ---
      ...
      function ensureCss() {
        const tagId = '@deepseek-ai/dsh-workspace-files/client.css';
      ...
      const folderSvg = (className) => React.createElement('svg', { className, width: 14, height: 14, viewBox: '0 0 16 16', 'aria-hidden': true }, React.createElement('path', { d: 'M1.5 3.5A1.5 1.5 0 0 1 3 2h3.2c.4 0 .78.16 1.06.44L8.5 3.7h4.5A1.5 1.5 0 0 1 14.5 5.2v7.3a1.5 1.5 0 0 1-1.5 1.5H3a1.5 1.5 0 0 1-1.5-1.5z', fill: 'currentColor' }));

      function fileGlyph(name) {
        const dot = name.lastIndexOf('.');
        const ext = dot < 0 ? '' : name.slice(dot + 1).toLowerCase();
        const G = (label, color) => ({ label, color });
        if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'bmp', 'ico', 'avif'].includes(ext)) return G('IMG', '#4fc1e9');
        if (['zip', 'tar', 'gz', 'bz2', 'xz', '7z', 'rar', 'tgz'].includes(ext)) return G('ZIP', '#d39a62');
        if (['json', 'jsonl', 'jsonc'].includes(ext)) return G('{}', '#cbcb41');
        if (['yaml', 'yml', 'toml'].includes(ext)) return G('YM', '#a181f7');
        if (['xml', 'plist'].includes(ext)) return G('XM', '#e8995e');
        if (['md', 'markdown', 'mdown'].includes(ext)) return G('MD', '#519aba');
        if (['txt', 'text', 'log'].includes(ext)) return G('TX', '#8b9bb4');
        if (['js', 'mjs', 'cjs'].includes(ext)) return G('JS', '#f7df1e');
        if (ext === 'jsx') return G('JX', '#f7df1e');
        if (ext === 'ts') return G('TS', '#3178c6');
        if (ext === 'tsx') return G('T+', '#3178c6');
        if (ext === 'py') return G('PY', '#3776ab');
        if (ext === 'go') return G('GO', '#00add8');
        if (ext === 'rs') return G('RS', '#dea584');
        if (ext === 'java') return G('JA', '#b07219');
        if (ext === 'kt' || ext === 'kts') return G('KT', '#7f52ff');
        if (ext === 'c' || ext === 'h') return G('C', '#a8b9cc');
        if (['cpp', 'cc', 'cxx', 'hpp', 'hh'].includes(ext)) return G('C+', '#a8b9cc');
        if (ext === 'cs') return G('C#', '#68217a');
        if (ext === 'rb') return G('RB', '#cc342d');
        if (ext === 'php') return G('PH', '#777bb4');
        if (ext === 'swift') return G('SW', '#ff7f50');
        if (['sh', 'bash', 'zsh', 'fish'].includes(ext)) return G('SH', '#89e051');
        if (ext === 'sql') return G('SQ', '#e38c00');
        if (ext === 'vue') return G('VU', '#42b883');
        if (ext === 'svelte') return G('SV', '#ff3e00');
        if (ext === 'css') return G('CS', '#42a5f5');
        if (['scss', 'sass', 'less'].includes(ext)) return G('SC', '#c6538c');
        if (ext === 'html' || ext === 'htm') return G('HT', '#e44d26');
        if (['ini', 'cfg', 'conf', 'env', 'properties'].includes(ext)) return G('CF', '#a181f7');
        if (ext === 'lock') return G('LK', '#8b8b8b');
        if (ext === 'pdf') return G('PD', '#d53f3f');
        if (['doc', 'docx'].includes(ext)) return G('DO', '#2b579a');
        if (['xls', 'xlsx', 'csv'].includes(ext)) return G('XL', '#217346');
        if (['ppt', 'pptx'].includes(ext)) return G('PP', '#d24726');
        if (['mp3', 'wav', 'ogg', 'm4a', 'flac'].includes(ext)) return G('AU', '#a56cc1');
        if (['mp4', 'mov', 'mkv', 'avi', 'webm'].includes(ext)) return G('VI', '#7b52ab');
        if (name === 'Dockerfile') return G('DK', '#2496ed');
        if (name === 'LICENSE' || name === 'LICENSE.md') return G('LI', '#8b9bb4');
        return G('', '#8b9bb4');
      }
      ...
      function fileIcon(name) {
        ...
        return React.createElement('svg', { className: 'dshfm-icon', width: 16, height: 16, viewBox: '0 0 16 16', 'aria-hidden': true },
          React.createElement('path', { d: 'M4.5 1.5h4.4l3.6 3.6v8.9a1 1 0 0 1-1 1h-7a1 1 0 0 1-1-1V2.5a1 1 0 0 1 1-1z', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2 }),
          g.label === '' ? null : React.createElement('text', { x: 8, y: 11.6, textAnchor: 'middle', fontSize: 5.4, fontWeight: 700, fill: g.color, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }, g.label),
        );
      }
      ...
      function detectLang(name) {
        ...
        if (['c', 'h', 'cpp', 'cc', 'cxx', 'hpp', 'hh', 'cs', 'java', 'kt', 'kts', 'swift'].includes(ext)) return 'c';
        ...
        if (['xml', 'plist', 'svg'].includes(ext)) return 'xml';
        if (['ini', 'cfg', 'conf', 'env', 'properties'].includes(ext)) return 'ini';
        return 'plain';
      }
      ...
      function hlFor(lang) {
        if (HL_CACHE[lang] !== undefined) return HL_CACHE[lang];
        ...
        HL_CACHE[lang] = { re, colors: rules.map((r) => r[1]) };
      ...
      function tokenizeLine(line, hl) {
        ...
        while ((m = hl.re.exec(line)) !== null) {
          ...
          if (hl.re.lastIndex <= m.index) hl.re.lastIndex = m.index + 1;
        }
      ...
      function FilesView(props) {
        ...
        React.useEffect(() => {
          ...
          rpc('/dsh-workspace-files/list', { session: sid })
            .then((res) => {
              if (!alive) return;
              if (res && res.ok === true) {
                const entries = Array.isArray(res.entries) ? res.entries : [];
                setState({ phase: 'ready', root: typeof res.root === 'string' ? res.root : '', entries, error: '' });
                setDirData((d) => ({ ...d, '': entries }));
              } else {
                setState({ phase: 'error', root: '', entries: [], error: res && res.error ? String(res.error) : 'UNKNOWN' });
              }
            })
            .catch((error) => {
              if (!alive) return;
              setState({ phase: 'error', root: '', entries: [], error: error && error.message ? String(error.message) : 'UNKNOWN' });
            });
          return () => { alive = false; };
        }, [sid, reloadKey]);
        ...
        React.useEffect(() => {
          ...
          rpc('/dsh-workspace-files/read', { session: sid, path: preview.path })
            .then((res) => {
              if (!alive) return;
              if (res && res.ok === true) {
                setDoc({ phase: 'ready', kind: typeof res.kind === 'string' ? res.kind : 'binary', text: typeof res.text === 'string' ? res.text : '', dataUrl: typeof res.dataUrl === 'string' ? res.dataUrl : '', size: typeof res.size === 'number' ? res.size : 0, truncated: res.truncated === true, error: '', forPath: preview.path });
              } else {
                setDoc({ phase: 'error', kind: '', text: '', dataUrl: '', size: 0, truncated: false, error: res && res.error ? String(res.error) : 'UNKNOWN', forPath: preview.path });
              }
            })
            .catch((error) => {
              if (!alive) return;
              setDoc({ phase: 'error', kind: '', text: '', dataUrl: '', size: 0, truncated: false, error: error && error.message ? String(error.message) : 'UNKNOWN', forPath: preview.path });
            });
          return () => { alive = false; };
        }, [preview, sid]);
        ...
        React.useEffect(() => {
          ...
          rpc('/dsh-workspace-files/search', { session: sid, q })
            .then((res) => {
              if (!alive) return;
              if (res && res.ok === true) {
                setSearch({ phase: 'ready', query: q, matches: Array.isArray(res.matches) ? res.matches : [], truncated: res.truncated === true });
              } else {
        ...
        const fetchDir = (path) => {
          ...
          rpc('/dsh-workspace-files/dir', { session: sid, path })
            .then((res) => {
              setDirData((d) => ({ ...d, [path]: res && res.ok === true && Array.isArray(res.entries) ? res.entries : [] }));
            })
            .catch(() => {
              setDirData((d) => ({ ...d, [path]: [] }));
            })
        ...
        function renderRow(node) {
          if (node.type === 'dir') {
            return React.createElement('div', { key: node.path || node.name, className: 'dshfm-row', onClick: () => navigate(node.path), title: node.path || node.name },
          ...
          return React.createElement('div', { key: node.path || node.name, className: cls, onClick: () => openPreview(node), onDoubleClick: () => openPath(node.abs), title: node.path || node.name },
            React.createElement('span', { className: 'dshfm-chev' }, ''),
            fileIcon(node.name),
            React.createElement('span', { className: 'dshfm-name' }, node.name),
        ...
        function renderCrumbs() {
          ...
          const items = [];
          items.push(React.createElement('button', { key: 'root', type: 'button', className: 'dshfm-crumb', disabled: currentPath === '', onClick: () => navigate(''), title: state.root || '工作区' }, rootLabel));
          if (currentPath !== '') {
            ...
            for (let i = 0; i < parts.length; i += 1) {
              ...
              items.push(React.createElement('button', { key: 'c' + i, type: 'button', className: 'dshfm-crumb', disabled: isLast, onClick: () => navigate(p) }, parts[i]));
            }
          }
          return items;
        }

        const curEntries = dirData[currentPath] !== undefined ? dirData[currentPath] : null;
        ...
        if (q !== '') {
          if (search.phase === 'loading') {
            ...
            treeBody = search.matches.map((m) => {
              if (m.type === 'dir') {
                return React.createElement('div', { key: 'd' + m.path, className: 'dshfm-row', onClick: () => { navigate(m.path); setFilter(''); }, title: m.path },
                  folderSvg('dshfm-icon dshfm-icon-folder'),
              ...
              return React.createElement('div', { key: 'f' + m.path, className: cls, onClick: () => openPreview(m), onDoubleClick: () => openPath(m.abs), title: m.path },
                fileIcon(m.name),
                React.createElement('span', { className: 'dshfm-name' }, m.path),
                React.createElement('span', { className: 'dshfm-size' }, fmtSize(m.size)),
              );
            });
            if (search.truncated) treeBody = React.createElement(React.Fragment, null, treeBody, React.createElement('div', { className: 'dshfm-note', style: { borderTop: 'none' } }, '匹配过多，仅显示前 200 条'));
          }
        ...
        } else if (curEntries === null || curPending) {
          treeBody = React.createElement('div', { className: 'dshfm-center' }, '加载中…');
        ...
        function renderPreviewBody() {
          ...
          if (doc.kind === 'text') {
            const lang = detectLang(preview.name);
            const hl = hlFor(lang);
        ...
        let note = null;
        if (preview !== null && doc.phase === 'ready' && doc.kind === 'text' && (doc.truncated || doc.text.split('\n').length > 3000)) note = React.createElement('div', { className: 'dshfm-note' }, '内容过长，仅预览前 256 KB');
